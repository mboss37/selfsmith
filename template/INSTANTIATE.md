...
- [ ] `{{MISSION}}`: one paragraph on what the loop is optimizing and why it matters
...
- [ ] `{{ONE_LINE_BIO}}`: one sentence on who this persona is and what they care about
...
- [ ] **Unattended (any OS)**: schedule the bundled `run-iteration.sh` (NOT raw `claude`) on a
  ...
  security boundary from `sandbox/` (container / hardened systemd unit / macOS Seatbelt; see
